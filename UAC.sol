...
contract Management is Ownable(msg.sender) {
     ...
     */
    function getSessionDetails(uint256 _sessionId) public view returns (Session memory) {
...
contract NFTManager is ERC721URIStorage, Ownable(msg.sender) {
    ...
    event NFTMinted(address indexed user, uint256 patientId, uint256 tokenId); 
    event NFTBurned(address indexed user, uint256 tokenId);

    constructor(address _managementContract, address _ipfsValidator) ERC721("TherapySessionNFT", "TSNFT") {
        tokenId = 0;
        managementContract = Management(_managementContract);
        ipfsValidator = IPFSValidator(_ipfsValidator);
    }

    /**
     * @notice Mints an NFT for a patient who is registered in Management.sol.
     * @param patient The address of the patient to receive the NFT.
     * @param _tokenURI The metadata URI associated with the NFT.
     */
    function mintNFT(address patient, string memory _tokenURI) external onlyOwner {
        uint256 patientId = managementContract.getPatientId(patient);
        require(patientId > 0, "Patient is not registered in Management contract.");
        require(ipfsValidator.isValidIPFSHash(_tokenURI), "Invalid IPFS hash provided.");
        
        uint256 currentTokenId = tokenId;
        tokenId++;
        
        // Set token URI first before external calls
        _setTokenURI(currentTokenId, _tokenURI);
        
        // Emit event before the external call to avoid reentrancy risk
        emit NFTMinted(patient, patientId, currentTokenId);

        // Perform the minting after all state changes and event emission to prevent reentrancy
        _safeMint(patient, currentTokenId);
    }   

    /**
     * @notice Burns the NFT and unregisters the patient.
     * @param patient The address of the patient whose NFT should be burned.
     */
    function burnNFT(address patient) external onlyOwner {
        require(managementContract.getPatientId(patient) > 0, "Patient is not registered in Management contract.");

        uint256 currentTokenId = nftOwners[patient];
        _burn(currentTokenId);
        delete nftOwners[patient];

        emit NFTBurned(patient, currentTokenId);
    }

    /**
     * @notice Retrieves the token ID of the NFT owned by the patient.
     * @param patient The address of the patient.
     * @return The token ID of the NFT.
     */
    function getTokenId(address patient) external view returns (uint256) {
        require(managementContract.getPatientId(patient) > 0, "Patient is not registered in Management contract.");
        return nftOwners[patient];
    }
}
